...
const server = setupServer(
    ...
    rest.post('/api/orders/1/1', (req, res, ctx) => {
        return res(ctx.json(
        ))
    }),
...
it('HandOut renders without crashing', () => {
    const div = document.createElement('div');
    ReactDOM.render(<HandOut />, div);
});

//BASIC TEST

it("Each field should be in the document and free of text", () => {
    const utils = render(<HandOut />)
...
it("Orders should be shown after search", async () => {
    ...
    expect(screen.queryByText('status : booked')).not.toBeInTheDocument();
    expect(screen.queryByText('hand out')).not.toBeInTheDocument();
    ...
    await waitFor(() => {
        ...
        screen.getByText('status : booked')
        screen.getByText('hand out')
    })

    expect(screen.queryByText('id : 1')).toBeInTheDocument();
    expect(screen.queryByText('price : 10')).toBeInTheDocument();
    expect(screen.queryByText('status : booked')).toBeInTheDocument();
...
it("Alert should be shown if there is not orders", async () => {
    ...
    expect(screen.getByRole('alert')).toHaveTextContent("No orders found")
});
...
it("Shop Employee should be albe to hand out an order", async () => {

    render(<HandOut changeAction = {changeAction}/>);
    //get da errore se non trova l'elemento, bisogna usare queryBy
