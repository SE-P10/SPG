...
const getRequestedProducts = async (farmerID) => {
  ...
  return getQuerySQL(
    db,
    "SELECT P.id, SUM(OP.quantity) AS quantity, P.price, PD.name FROM order_product AS OP, products AS P, products_details AS PD WHERE OP.product_id = PD.id AND P.details_id = PD.id AND P.farmer_id = ? AND OP.order_id IN (SELECT DISTINCT id FROM orders WHERE status IN ('booked', 'pending')) GROUP BY P.id, PD.name,P.price",
    [farmerID],
    {
      id: 0,
      quantity: 0,
